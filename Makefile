...
# 1) run make once to create and update the po/visualitemmodel.pot
# 2) copy the template file and set the name to the language you want to
#    translate to:   cp po/visualitemmodel.pot po/en.po
# 3) edit the po file
...
APP_ID=visualitemmodel.liu-xiao-guo
...
po/visualitemmodel.pot: $(QMLJS_FILES)
	mkdir -p $(CURDIR)/po && xgettext -o po/visualitemmodel.pot --qt --c++ --add-comments=TRANSLATORS --keyword=tr --keyword=tr:1,2 $(QMLJS_FILES) --from-code=UTF-8
